process.on('unhandledRejection', error => {
	console.error("Yakalanamayan bir hata oluştu: ", error);
	if (error instanceof Error) {
	  console.error("Hata detayları:", error.stack || error.message);
	}
  });
...
try {
	...
	console.error('Bot login sırasında bir hata oluştu:', error);
  }
...
client.on('messageCreate', async (message) => {
	...
	if (!isSubscribed) {
		...
		const embed = new EmbedBuilder()
			...
			.setAuthor({ name: message.author.tag, iconURL: message.author.displayAvatarURL() })
			.setDescription(`❌ **|** Hey! Kanala abone olmamışsın veya ben okuyamadım lütfen tam ekran ss at !`)
			.setTimestamp()
			.setFooter({ text: `${client.user?.username} © 2023`, iconURL: client.user?.displayAvatarURL() })
